...
Source: https://devdocs.io/webpack~5/configuration/experiments#experiments
...
- Type:
  - (string | RegExp | ((uri: string) => boolean))[] A shortcut for experiments.buildHttp.allowedUris.
  - HttpUriOptions {
...
- 5.87.0+ Style-specific fields resolution in package.json files: webpack will look for style field in package.json files and use that if it exists for imports inside CSS files. For example, if you add @import 'bootstrap'; to your CSS file, webpack will look for bootstrap in node_modules and use the style field in package.json from there. If style field is not found, webpack will use the main field instead to preserve backward compatibility.
