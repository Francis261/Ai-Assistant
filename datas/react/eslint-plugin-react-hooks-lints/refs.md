...
Source: https://devdocs.io/react/reference/eslint-plugin-react-hooks/lints/refs
...
- Returned from useRef() or React.createRef(). const scrollRef = useRef(null);
- An identifier named ref or ending in Ref that reads from or writes to .current. buttonRef.current = node;
- Passed through a JSX ref prop (for example <div ref={someRef} />). <input ref={inputRef} />
