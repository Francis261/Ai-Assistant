...
Source: https://devdocs.io/javascript/lexical_grammar
...
- Write postfix ++ and -- on the same line as their operands. const a = b
++
console.log(a) // ReferenceError: Invalid left-hand side expression in prefix operation
 const a = b++
console.log(a)
- The expressions after return, throw, or yield should be on the same line as the keyword. function foo() {
 return
...
- Similarly, the label identifier after break or continue should be on the same line as the keyword. outerBlock: {
 innerBlock: {
 break
...
- The => of an arrow function should be on the same line as the end of its parameters. const foo = (a, b)
 => a + b
 const foo = (a, b) =>
 a + b
- The async of async functions, methods, etc. cannot be directly followed by a line terminator. async
function foo() {}
 async function
foo() {}
- The using keyword in using and await using statements should be on the same line as the first identifier it declares. using
resource = acquireResource()
 using resource
 = acquireResource()
- If a line starts with one of (, [, `, +, -, / (as in regex literals), prefix it with a semicolon, or end the previous line with a semicolon. // The () may be merged with the previous line as a function call
(() => {
 // …
})()
...
;-a.toString()
;/pattern/.exec(str).forEach(console.log)
- Class fields should preferably always be ended with semicolons — in addition to the previous rule (which includes a field declaration followed by a computed property, since the latter starts with [), semicolons are also required between a field declaration and a generator method. class A {
 a = 1
 [b] = 2
