...
Source: https://devdocs.io/vite/guide/features
...
```
import  from './example.module.css'
.('foo'). = .
```
...
import  from './bar.css?inline' // will not be injected
```
...
import  from './asset.js?url'
```

```
// Load assets as strings
import  from './shader.glsl?raw'
```

```
// Load Web Workers
import  from './worker.js?worker'
```

```
// Web Workers inlined as base64 strings at build time
import  from './worker.js?worker&inline'
```
...
```
const  = import.meta.(['./dir/*.js', './another/*.js'])
```
...
```
const  = import.meta.(['./dir/*.js', '!**/bar.js'])
```
...
```
const  = import.meta.('./dir/*.js', { : 'setup' })
```
...
const  = async () => {
  const  = (await import('./test/boot.test.wasm?url')).
  const  = await ('.' + )
  const {  } = await WebAssembly.(, {
    /* ... */
  })
  /* ... */
}
...
```
import  from './worker?worker&inline'
```

If you wish to retrieve the worker as a URL, add the url query:

```
import  from './worker?worker&url'
```
