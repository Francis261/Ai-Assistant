...
Source: https://devdocs.io/npm/creating-and-viewing-access-tokens
...
8. (Optional) In the Organizations section, configure your token's access to organizations. Note: When you give a token access to an organization, the token can only be used for managing organization settings and teams or users associated with the organization. It does not give the token the right to publish packages managed by the organization.
  - In the Permissions dropdown menu, select No access, Read-only, or Read and write.
