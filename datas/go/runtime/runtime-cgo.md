...
Source: https://devdocs.io/go/runtime/cgo/index
...
callbacks.go callbacks_traceback.go cgo.go clearenv.go handle.go iscgo.go linux.go mmap.go setenv.go sigaction.go
...
Note that, as described in the cmd/cgo documentation, the C code must not keep a copy of the Go pointer that it receives, unless the memory is explicitly pinned using runtime.Pinner. This example is OK because the C function myprint does not keep a copy of the pointer.
