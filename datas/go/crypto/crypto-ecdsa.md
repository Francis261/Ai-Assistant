...
Source: https://devdocs.io/go/crypto/ecdsa/index
...
ecdsa.go ecdsa_legacy.go notboring.go
...
Note that private keys are more commonly encoded in ASN.1 or PKCS#8 format, which can be generated with crypto/x509.MarshalECPrivateKey or crypto/x509.MarshalPKCS8PrivateKey (and encoding/pem).
...
### func ParseUncompressedPublicKey 1.25
...
Note that public keys are more commonly encoded in DER (or PEM) format, which can be generated with crypto/x509.MarshalPKIXPublicKey (and encoding/pem).
